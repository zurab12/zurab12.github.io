...
var hostkey = 'http://arkmv.ru:2053'.replace('http://', '').replace('https://', '');
...
window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
  if (!window.rch_nws[hostkey].startTypeInvoke) {
    ...
    if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
      ...
      net.silent('http://arkmv.ru:2053'.indexOf(location.host) >= 0 ? 'https://github.com/' : host + '/cors/check', function() {
        check(true);
      }, function() {
        check(false);
      }, false, {
...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  window.rch_nws[hostkey].typeInvoke('http://arkmv.ru:2053', function() {
          ...
          new Response(compressedStream).arrayBuffer()
            .then(function(compressedBuffer) {
              ...
              if (compressedArray.length > html.length) {
                ...
                $.ajax({
                  url: 'http://arkmv.ru:2053/rch/gzresult?id=' + rchId,
                  type: 'POST',
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript(["http://arkmv.ru:2053/js/nws-client-es5.js?v18112025"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  function component(object) {
    ...
    if (balansers_with_search == undefined) {
      ...
      network.silent(account('http://arkmv.ru:2053/lite/withsearch'), function(json) {
  ...
  if (false) {
    if (Platform.is('browser') && location.host.indexOf("127.0.0.1") !== -1) {
      ...
      $.get('http://arkmv.ru:2053/player-inner/' + element.url);
      return;
    }
