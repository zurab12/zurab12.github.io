(function() {
  ...
  var Defined = {
    api: 'lampac',
    localhost: 'https://beta.l-vid.online/lite/withsearch?uid=zdpc0ufg/lite/events?life=true&id=1426964&imdb_id=tt39369643&title=%D0%93%D1%80%D1%83%D0%BF%D0%BF%D0%B8%D1%80%D0%BE%D0%B2%D0%BA%D0%B0%3A%20%D0%BE%D0%B1%D1%89%D0%B8%D0%B9%20%D1%81%D0%B1%D0%BE%D1%80&original_title=Salve%20Geral%3A%20Irmandade&serial=0&original_language=pt&year=2026&source=tmdb&clarification=0&similar=false&rchtype=cors/',
    apn: ''
  };
...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  function startPlugin() {
				...
				if(el.yt_call_url){
					...
					ytNet.silent(account(el.yt_call_url + '&rjson=true'), function(json){
						if(json && json.data && json.data.length && json.data[0].stream){
							...
							if(d.quality) playdata.quality = d.quality;
							if(d.qualitys) playdata.qualitys = d.qualitys;
							if(d.hls_manifest_timeout) playdata.hls_manifest_timeout = d.hls_manifest_timeout;
	...
	// --- YouTube Feed Component (sidebar menu → discovery page) ---
	(function(){
		var ytFeedCategories = [
			{title: 'Музыка', query: 'Музыка'},
			{title: 'Игры', query: 'Игры'},
			{title: 'Фильмы', query: 'Фильмы'},
			{title: 'Новости', query: 'Новости'},
			{title: 'Спорт', query: 'Спорт'},
			{title: 'Наука', query: 'Наука'},
			{title: 'Юмор', query: 'Юмор'}
		];
		...
		function YouTubeFeed(object){
			...
			var html    = $('<div></div>');
			var body    = $('<div class="ytfeed-body"></div>');
			var searchBox = $('<div class="ytfeed-search"><input class="ytfeed-search__input selector" type="text" placeholder="Поиск на YouTube..." /></div>');
			var last    = false;
			var items   = [];
			var _this   = this;
			var activeRequests = [];
			...
			this.create = function(){
				...
				// Load default categories one by one
				loadCategories();
			...
			function cancelAll(){
				activeRequests.forEach(function(r){ try { r.clear(); } catch(e){} });
				activeRequests = [];
			}
			...
			function loadCategories(){
				...
				var loaded = 0;
				var total = ytFeedCategories.length;

				ytFeedCategories.forEach(function(cat){
					var net = new Lampa.Reguest();
					activeRequests.push(net);

					// Placeholder row that will be filled when data arrives
					var line = $('<div class="ytfeed-line" data-cat="' + cat.query + '"></div>');
					var head = $('<div class="ytfeed-line__head"><div class="ytfeed-line__title">' + cat.title + '</div></div>');
					var row  = $('<div class="ytfeed-line__row"></div>');
					var loader = $('<div class="ytfeed-line__loader"><div class="broadcast__scan"><div></div></div></div>');
					line.append(head);
					line.append(row);
					row.append(loader);
					body.append(line);
					...
					net.timeout(60000);
					net.silent(account(Defined.localhost + 'lite/youtube/feed?category=' + encodeURIComponent(cat.query)), function(json){
						loaded++;
						row.find('.ytfeed-line__loader').remove();
						if(json && json.categories && json.categories.length && json.categories[0].results){
							fillRow(row, json.categories[0].results);
						} else {
							row.append('<div class="ytfeed-line__empty">Нет результатов</div>');
						}
						if(loaded === 1){
						...
						loaded++;
						row.find('.ytfeed-line__loader').remove();
						row.append('<div class="ytfeed-line__empty">Ошибка</div>');
						if(loaded === 1){
							_this.activity.loader(false);
							_this.activity.toggle();
						}
			...
			function loadSearch(query){
				...
				activeRequests.push(net);
				net.timeout(60000);
				net.silent(account(Defined.localhost + 'lite/youtube/feed?search=' + encodeURIComponent(query)), function(json){
					_this.activity.loader(false);
					if(json && json.categories && json.categories.length && json.categories[0].results){
						appendCategory(json.categories[0].title, json.categories[0].results);
					} else {
						var empty = new Lampa.Empty({title: '', descr: 'Нет результатов'});
						body.append(empty.render());
					}
					_this.activity.toggle();
				}, function(){
					_this.activity.loader(false);
					var empty = new Lampa.Empty({title: '', descr: 'Ошибка загрузки'});
					body.append(empty.render());
					_this.activity.toggle();
				}, false, {headers:{'X-Kit-AesGcm': Lampa.Storage.get('aesgcmkey','')}});
			}

			function fillRow(row, results){
			...
			function appendCategory(title, results){
				var line = $('<div class="ytfeed-line"></div>');
				var head = $('<div class="ytfeed-line__head"><div class="ytfeed-line__title">' + title + '</div></div>');
				var row  = $('<div class="ytfeed-line__row"></div>');
				line.append(head);
				line.append(row);
				results.forEach(function(item){
			...
			function makeCard(item){
				var card = $('<div class="ytfeed-card selector"></div>');
				var view = $('<div class="ytfeed-card__view"></div>');
				var img  = $('<img class="ytfeed-card__img" />');
				img.attr('src', item.img || '');
				img.on('load', function(){ $(this).addClass('loaded'); });
				view.append(img);

				if(item.duration){
					view.append('<div class="ytfeed-card__duration">' + item.duration + '</div>');
				}

				var info = $('<div class="ytfeed-card__info"></div>');
				info.append('<div class="ytfeed-card__title">' + (item.title || '') + '</div>');
				if(item.channel){
					info.append('<div class="ytfeed-card__channel">' + item.channel + '</div>');
				}
				...
				card.on('hover:enter', function(){
					...
					var playNet = new Lampa.Reguest();
					playNet.timeout(120000);
					playNet.silent(account(item.url + '&rjson=true'), function(json){
						if(json && json.data && json.data.length && json.data[0].stream){
							var d = json.data[0];
							var playdata = {
								title: item.title || '',
		...
		// CSS for YouTube feed
		Lampa.Template.add('ytfeed_style', "<style>\
.ytfeed-search { padding: 1em 1.5em; }\
.ytfeed-search__input { width: 100%; padding: .7em 1em; font-size: 1.1em; background: rgba(255,255,255,.08); border: 2px solid transparent; border-radius: .3em; color: #fff; outline: none; }\
.ytfeed-search__input.focus { border-color: #fff; background: rgba(255,255,255,.15); }\
.ytfeed-line { margin-bottom: 1.5em; }\
.ytfeed-line__head { padding: 0 1.5em .5em; }\
.ytfeed-line__title { font-size: 1.5em; font-weight: 600; color: #fff; }\
.ytfeed-line__row { display: flex; flex-wrap: nowrap; overflow-x: auto; padding: 0 1.5em; gap: 1em; }\
.ytfeed-card { flex-shrink: 0; width: 18em; cursor: pointer; position: relative; }\
.ytfeed-card.focus .ytfeed-card__view { outline: .2em solid #fff; border-radius: .3em; }\
.ytfeed-card__view { position: relative; width: 100%; padding-bottom: 56.25%; background: rgba(255,255,255,.06); border-radius: .3em; overflow: hidden; }\
.ytfeed-card__img { position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover; opacity: 0; transition: opacity .3s; }\
.ytfeed-card__img.loaded { opacity: 1; }\
.ytfeed-card__duration { position: absolute; bottom: .4em; right: .4em; background: rgba(0,0,0,.8); color: #fff; font-size: .85em; padding: .1em .4em; border-radius: .2em; }\
.ytfeed-card__info { padding: .4em 0; }\
.ytfeed-card__title { font-size: 1.05em; color: #fff; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; line-height: 1.3; }\
.ytfeed-card__channel { font-size: .85em; color: rgba(255,255,255,.5); margin-top: .2em; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }\
.ytfeed-line__loader { padding: 1em 0; }\
.ytfeed-line__loader .broadcast__scan { height: .3em; background: rgba(255,255,255,.1); border-radius: .2em; overflow: hidden; }\
.ytfeed-line__loader .broadcast__scan > div { height: 100%; width: 30%; background: rgba(255,255,255,.3); border-radius: .2em; animation: ytfeed-scan 1.5s ease-in-out infinite; }\
@keyframes ytfeed-scan { 0% { transform: translateX(-100%); } 100% { transform: translateX(400%); } }\
.ytfeed-line__empty { padding: .5em 0; color: rgba(255,255,255,.4); font-size: 1em; }\
</style>");
		...
		if(window.appready) addYTMenu();
		else {
			Lampa.Listener.follow('app', function(e){
				if(e.type == 'ready'){
					addYTMenu();
					$('body').append(Lampa.Template.get('ytfeed_style', {}, true));
				}
			});
		}

		// Inject styles immediately if app already loaded
		if(window.appready) $('body').append(Lampa.Template.get('ytfeed_style', {}, true));
	})();
