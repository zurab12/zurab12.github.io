...
if (!window.rch_nws || !window.rch_nws[hostkey]) {
  ...
  window.rch_nws[hostkey] = {
    тип: Lampa.Platform.is('android') ? 'apk' : Lampa.Platform.is('tizen') ? 'cors' : undefined,
...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  window.rch_nws[hostkey].typeInvoke('https://lampaua.mooo.com', function() {
    ...
    client.invoke("RchRegistry", JSON.stringify({
      ...
      игрок: Lampa.Storage.field('player'),
    ...
    client.on("RchClient", function(rchId, url, data, headers, returnHeaders) {
      ...
      function result(html) {
        ...
        if (typeof CompressionStream !== 'undefined' && html && html.length > 1000) {
          ...
          вар кодировщик = новый TextEncoder();
          var readable = new ReadableStream({
            начало: функция(контроллер) {
          ...
          new Response(compressedStream).arrayBuffer()
            .then(function(compressedBuffer) {
              вар compressedArray = новый Uint8Array (compressedBuffer);
      ...
      if (url == 'eval') {
        ...
        network["native"](url, result, function(e) {
          console.log('RCH', 'результат пуст, ' + e.status);
          результат('');
        }, данные, {
          Тип данных: 'текст',
          таймаут: 1000 * 8,
    ...
    client.on('Закрыто', function() {
      console.log('RCH', 'Соединение закрыто');
    });
  ...
  функция component(object) {
	...
	функция clarificationSearchAdd(value){
	...
	функция clarificationSearchDelete(){
		var id = Lampa.Utils.hash(object.movie.number_of_seasons ? object.movie.original_name : object.movie.original_title);
		var all = Lampa.Storage.get('clarification_search','{}');
    ...
    this.startSource = function(json) {
      return new Promise(function(resolve, reject) {
        ...
        if (filter_sources.length) {
          ...
          Lampa.Storage.set('active_balanser', балансировщик);
          resolve(json);
        } еще {
          отклонять();
        }
        ...
        вар плавник = функция плавник(вызов) {
          ...
          network.silent(account(url), function(json) {
            ...
            filter.set('sort', filter_sources.map(function(e) {
              возвращаться {
                заголовок: источники[e].имя,
                источник: e,
                выбрано: e == балансировщик,
    ...
    this.toPlayElement = function(file) {
      var play = {
        заголовок: файл.заголовок,
        URL: file.url,
        качество: файл.qualitys,
        временная шкала: файл.timeline,
        субтитры: файл.субтитры,
		сегменты: файл.сегменты,
        обратный вызов: file.mark,
		сезон: файл.сезон,
		эпизод: файл.эпизод,
		voice_name: file.voice_name,
		миниатюра: файл.миниатюра
      };
      Возврат игры;
    };
      ...
      this.getEpisodes(items[0].season, function(episodes) {
        ...
        items.forEach(function(element, index) {
          ...
          Lampa.Arrays.extend(element, {
            ...
            качество: '',
            время: Lampa.Utils.secondsToTime((episode ? episode.runtime : object.movie.runtime) * 60, true)
          });
        ...
        if (serial && episodes.length > items.length && !params.similars) {
          ...
          left.forEach(function(episode) {
            ...
            var html = Lampa.Template.get('lampac_prestige_full', {
              ...
              заголовок: episode.name,
              качество: день > 0 ? текст : ''
            });
            var loader = html.find('.online-prestige__loader');
            var image = html.find('.online-prestige__img');
            ...
            onSelect: function onSelect(a) {
              ...
              if (a.subscribe) {
                Lampa.Account.subscribeToTranslation({
                  карта: объект.фильм,
                  сезон: params.element.season,
                  эпизод: params.element.translate_episode_end,
                  голос: params.element.translate_voice
                }, function() {
    ...
    var source = {
      ...
      onSelect: function(params, close) {
        ...
        Lampa.Activity.push({
          ...
          компонент: 'BazarNetUA',
          фильм: params.element,
          страница: 1,
          поиск: params.element.title,
          уточнение: верно,
  ...
  function startPlugin() {
    ...
    var manifst = {
      тип: 'видео',
      версия: '',
      имя: 'BazarNetUA (Безкоштовно)',
      описание: 'https://lampaua.mooo.com (Безкоштовно)',
      компонент: 'BazarNetUA',
      ...
      onContextLauch: function onContextLauch(object {
        ...
        Lampa.Activity.push({
          ...
          компонент: 'BazarNetUA',
          поиск: все[id] ? все[id] : объект.title,
          search_one: object.title,
          search_two: object.original_title,
          фильм: объект,
          страница: 1,
		  уточнение: все[id] ? true : false
        });
      }
    };
    ...
    Lampa.Lang.add({
      ...
      lampac_no_watch_history: {
        ...
        en: 'История просмотров отсутствует',
      ...
      helper_online_file: { //
        ...
        uk: 'Уберите клавишу «ОК» для вызова контекстного меню',
        Нажмите и удерживайте клавишу «ОК», чтобы открыть контекстное меню.
        zh: '按住“确定”键调出上下文菜单'
      },
      title_online: { //
        ru: 'Онлайн',
      ...
      lampac_balanser_dont_work: { //
        ...
        en: 'Поиск по ({balanser}) не дал результатов',
      ...
      lampac_balanser_timeout: { //
        ...
        en: 'Источник будет автоматически переключен через <span class="timeout">10</span> секунд.',
    ...
    Lampa.Component.add('BazarNetUA', компонент); //то же самое
    resetTemplates();
    ...
    function addButton(e) {
      ...
      btn.on('hover:enter', function() {
        ...
        Lampa.Activity.push({
          ...
          заголовок: Lampa.Lang.translate('title_online'),
          компонент: 'BazarNetUA',
          поиск: все[id] ? все[id] : e.movie.title,
