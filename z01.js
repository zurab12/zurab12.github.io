(function() {
  ...
  var Defined = {
    api: 'lampac',
    localhost: 'http://ststst.keenetic.pro:777/',
    apn: ''
  };
...
var hostkey = 'http://ststst.keenetic.pro:777'.replace('http://', '').replace('https://', '');
...
window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
  if (!window.rch_nws[hostkey].startTypeInvoke) {
    ...
    if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
      ...
      net.silent('http://ststst.keenetic.pro:777'.indexOf(location.host) >= 0 ? 'https://github.com/' : host + '/cors/check', function() {
        check(true);
      }, function() {
        check(false);
      }, false, {
...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  window.rch_nws[hostkey].typeInvoke('http://ststst.keenetic.pro:777', function() {
    ...
    client.on("RchClient", function(rchId, url, data, headers, returnHeaders) {
	  ...
	  function sendResult(uri, html) {
	    $.ajax({
	      url: 'http://ststst.keenetic.pro:777/rch/' + uri + '?id=' + rchId,
	      type: 'POST',
	      data: html,
	      async: true,
	      cache: false,
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript(["http://ststst.keenetic.pro:777/js/nws-client-es5.js?v03022026"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  function component(object) {
    ...
    if (balansers_with_search == undefined) {
      ...
      network.silent(account('http://ststst.keenetic.pro:777/lite/withsearch'), function(json) {
